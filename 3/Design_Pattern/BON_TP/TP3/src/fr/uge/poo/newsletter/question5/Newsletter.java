...
public class Newsletter {
  ...
  public static class Builder {
    ...
    private Observer observer = new EmptyObserver();
  ...
  private void aftertInit() {
    for (var user : subscribers.values()) {
      observer.onSubscribeSuccess(name,  user.email(), subscribers.size());
      observer.onSubscribeListUpdate(name,  subscribers.values().stream().map(User::name).toList());
    }
  }
  ...
  public void subscribe(User user) {
    ...
    if (!userCondition.test(user)) {
      observer.onSubscribeError(name, user.name());
      throw new IllegalArgumentException("Condition failed");
    }
  ...
  public void sendMessage(String title, String content) {
    ...
    observer.onNewsLetterSend(name, title, subscribers.values().stream().map(User::email).toList());
  }
}
