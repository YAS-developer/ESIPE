...
public class STPParser {
	...
	public static Optional<STPCommand> parse(String cmd){
		...
		switch (tokens[0]){
		case "hello": 
			return Optional.of(new STPCommand.HelloCmd());
		case "start": 
			if (tokens.length!=2 || !isInteger(tokens[1])){
				return Optional.empty();
			}
			return Optional.of(new STPCommand.StartTimerCmd(Integer.parseInt(tokens[1])));
		case "stop": 
			if (tokens.length!=2 || !isInteger(tokens[1])){
				return Optional.empty();
			}
			return Optional.of(new STPCommand.StopTimerCmd(Integer.parseInt(tokens[1])));
		case "elapsed": 
			if (tokens.length==1){
				return Optional.empty();
			}
			...
			return Optional.of(new STPCommand.ElapsedTimeCmd(timersId));
		default:
			return Optional.empty();
		}
	}
